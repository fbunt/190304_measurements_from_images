...
class Camera(object):
    ...
    def transforms(self, X, p):
        ...
        #hom_coords = np.column_stack((X, new_col))
        #print(hom_coords)
        ...
        gen_coords = np.zeros((X.shape[0], 3))
...
class rw_coords:
    ...
    def transform(self, guess, cam1, cam2):
        u1 = cam1.transforms(np.array([guess]), cam1.p)[0, 0]
        v1 = cam1.transforms(np.array([guess]), cam1.p)[0, 1]
        u2 = cam2.transforms(np.array([guess]), cam2.p)[0, 0]
        v2 = cam2.transforms(np.array([guess]), cam2.p)[0, 1]

        return np.array([u1,v1,u2,v2]).flatten()
